...
function star (args, cb) {
  const config = StarConfig(npmConfig())
  return BB.try(() => {
    if (!args.length) throw new Error(star.usage)
    let s = config.get('unicode') ? '\u2605 ' : '(*)'
    const u = config.get('unicode') ? '\u2606 ' : '( )'
    const using = !(npm.command.match(/^un/))
    if (!using) s = u
    return BB.map(args.map(npa), pkg => {
      return BB.all([
        whoami([pkg], true, () => {}),
        fetch.json(pkg.escapedName, config.concat({
          spec: pkg,
          query: {write: true},
          'prefer-online': true
        }))
        ...
        return fetch.json(pkg.escapedName, config.concat({
          spec: pkg,
          method: 'PUT',
          body
        }))
